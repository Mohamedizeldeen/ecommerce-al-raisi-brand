<x-layouts.storefront title="{{ __('Create Account') }}">
    <section class="mx-auto max-w-md px-4 sm:px-6 py-16">
        <h1 class="mb-8 text-center text-3xl text-ink">{{ __('Create Account') }}</h1>

        <form method="POST" action="{{ route('register') }}" class="space-y-5">
            @csrf
            <x-storefront.field name="name" label="{{ __('Full name') }}" :value="old('name')" />
            ...
            <x-storefront.field name="password_confirmation" label="{{ __('Confirm password') }}" type="password" :required="true" />
        ...
        <p class="mt-6 text-center text-sm text-stone-500">
            {{ __('Already have an account?') }} <a href="{{ route('login') }}" class="text-accent hover:underline">{{ __('Sign in') }}</a>
        </p>
    </section>
