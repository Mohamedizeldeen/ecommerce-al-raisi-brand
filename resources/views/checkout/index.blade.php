<x-layouts.storefront title="{{ __('Checkout') }}">
            ...
            <div class="space-y-6 lg:col-span-2">
                <div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        @php($u = auth()->user())
                        <x-storefront.field name="customer_name" label="{{ __('Full name') }}" :value="old('customer_name', $u?->name)" />
                        <x-storefront.field name="customer_email" label="{{ __('Email') }}" type="email" :value="old('customer_email', $u?->email)" />
                        <x-storefront.field name="customer_phone" label="{{ __('Phone') }}" :value="old('customer_phone', $u?->phone)" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div class="sm:col-span-2"><x-storefront.field name="shipping_address_line1" label="{{ __('Address line 1') }}" :value="old('shipping_address_line1')" /></div>
                        <div class="sm:col-span-2"><x-storefront.field name="shipping_address_line2" label="{{ __('Address line 2 (optional)') }}" :value="old('shipping_address_line2')" :required="false" /></div>
                        <x-storefront.field name="shipping_city" label="{{ __('City') }}" :value="old('shipping_city')" />
                        <x-storefront.field name="shipping_region" label="{{ __('Region (optional)') }}" :value="old('shipping_region')" :required="false" />
                    </div>
                    <div class="mt-4">
