<x-layouts.storefront :title="__('Lookbooks')" :description="__('Explore the seasonal lookbooks and campaign collections of Amal Al Raisi — the stories behind the house.')">
    <section class="mx-auto max-w-7xl px-4 sm:px-6 py-16">
        <header class="mb-14 text-center">
            <p class="text-xs uppercase tracking-[0.3em] text-accent">{{ __('Editorial') }}</p>
            <h1 class="mt-3 text-4xl text-ink">{{ __('Lookbooks') }}</h1>
            <p class="mt-3 text-stone-500">{{ __('Our seasonal and campaign collections — the stories behind the house.') }}</p>
        </header>
                ...
                <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($group as $collection)
                        <div class="reveal" x-data x-intersect.once="$el.classList.add('is-visible')"
                            ...
                            <a href="{{ route('lookbooks.show', $collection) }}"
