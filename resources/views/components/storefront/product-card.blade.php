...
<a href="{{ route('products.show', $product) }}" class="group block">
    <div class="relative aspect-[4/5] overflow-hidden bg-sand">
        ...
        @unless ($product->in_stock)
            <span class="absolute left-3 top-3 z-10 bg-white/90 px-2 py-1 text-[10px] uppercase tracking-[0.18em] text-ink">{{ __('Sold Out') }}</span>
        @endunless
    ...
    <div class="mt-3 text-center">
        ...
        <p class="mt-1 text-sm text-stone-500">{{ money((int) $product->base_price_baisa) }}</p>
    </div>
</a>
