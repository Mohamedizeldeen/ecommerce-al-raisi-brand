<x-layouts.storefront title="Search">
    <section class="mx-auto max-w-7xl px-4 sm:px-6 py-12">
        <form method="GET" action="{{ route('search') }}" class="mx-auto max-w-xl">
            <div class="flex items-center border-b border-ink">
                <input type="search" name="q" value="{{ $term }}" placeholder="Search products…" autofocus
                    class="w-full bg-transparent py-3 text-lg placeholder-stone-400 focus:outline-none">
                <button type="submit" class="px-2 text-ink hover:text-accent" aria-label="Search">
        ...
        <div class="mt-12">
            @if ($term !== '')
                <p class="mb-8 text-sm text-stone-500">Results for &ldquo;<span class="text-ink">{{ $term }}</span>&rdquo;</p>
            @endif
